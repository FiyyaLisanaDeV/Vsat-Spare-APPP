...
export default function KategoriBarangPage() {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const res = await masterDataApi.getItemCategories({
      ...
      alert("Gagal memuat data kategori barang");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleToggle = async (item: any) => {
    if (confirm(`Yakin ingin ${item.is_active ? 'menonaktifkan' : 'mengaktifkan'} ${item.name}?`)) {
      try {
        await masterDataApi.toggleItemCategory(item.id);
        fetchData();
      } catch (error: any) {
  ...
  const columns = [
    { key: "name", label: "Nama Kategori", render: (item: any) => (
    ...
    { key: "type_name", label: "Jenis Barang" },
  ];
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Kelola master data Kategori Barang.</p>
      </div>
